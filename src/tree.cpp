...
namespace quickbook { namespace detail {
    ...
    tree_base::tree_base(tree_base&& x) :
        root_(x.root_)
    {
        x.root_ = 0;
    }
    ...
    tree_node_base* tree_base::extract(tree_node_base* x) {
        if (!x->prev_) {
            if (x->parent_) { x->parent_->children_ = x->next_; }
            else { assert(x == root_); root_ = x->next_; }
    ...
    tree_node_base* tree_builder_base::extract(tree_node_base* x) {
        if (!x->prev_) {
            if (x->parent_) { x->parent_->children_ = x->next_; }
            else { assert(x == root_); root_ = x->next_; parent_ = 0; current_ = root_; }
        } else { x->prev_->next_ = x->next_; }
        if (x->next_) { x->next_->prev_ = x->prev_; }
        x->parent_ = 0;
        x->next_ = 0;
